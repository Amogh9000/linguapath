...
export default function LoginPage() {
  ...
  const finishLogin = (token: string) => {
    ...
    if (typeof window !== 'undefined') {
      sessionStorage.setItem('lp_boot_loader', '1');
    }
    router.push('/learn');
  };
  ...
  const handleDemo = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.login({ email: 'demo@linguapath.com', password: 'demo1234' });
      finishLogin(res.access_token);
    } catch (err: any) {
      ...
      <main className="w-full max-w-5xl bg-white rounded-[2rem] overflow-hidden flex flex-col md:flex-row shadow-lg border-2 border-surface-container-highest">
        ...
        <div className="w-full md:w-1/2 p-6 md:p-12 flex flex-col justify-center bg-white relative">
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="h-[2px] bg-surface-container-highest flex-1"></div>
            <span className="text-sm font-black text-on-surface-variant uppercase">Or</span>
            <div className="h-[2px] bg-surface-container-highest flex-1"></div>
          </div>
          ...
          <button 
            onClick={handleDemo}
            disabled={loading}
            className="w-full flex items-center justify-center bg-primary-container text-on-primary py-3 rounded-2xl font-black text-sm btn-3d-primary uppercase tracking-widest hover:brightness-110 disabled:opacity-50"
          >
            ⚡ Quick Demo Play (Japanese)
          </button>

          <div className="mt-8 text-center font-bold text-on-surface">
            Don&apos;t have an account? 
            <Link href="/signup" className="text-secondary font-black hover:underline ml-1">
              Sign up
            </Link>
          </div>
        </div>
